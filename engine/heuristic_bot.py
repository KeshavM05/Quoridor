...
def generate_heuristic_games(num_games=50000, max_moves=80):
    """
    Generate training data by having the heuristic bot play against itself.
    Returns list of (state, policy, value) tuples.
    """
    import time

    print(f"Generating {num_games} heuristic bot games...")
    t0 = time.time()
    all_examples = []
    game_lengths = []
    wins = {1: 0, 2: 0, 0: 0}
    ...
    for game_idx in range(num_games):
        game = QuoridorGame()
        history = []  # (state, move_made, player)
        ...
        for t, (state, move, player) in enumerate(history):
            # Policy: high weight on the move chosen
            policy = np.zeros(ACTION_SIZE, dtype=np.float32)
            action = move_to_action(move)
            policy[action] = 1.0  # deterministic policy from heuristic

            # Value: discounted outcome
            if winner == 0:
                # Draw — use BFS distance
        ...
        if (game_idx + 1) % 1000 == 0:
            ...
            print(f"  {game_idx+1}/{num_games} games | avg length: {avg_len:.1f} | "
                  f"wins: P1={wins[1]}, P2={wins[2]}, draws={wins[0]} | "
                  f"{elapsed:.1f}s")
